...
body = issue.get("body", "")
item_ids = [value.strip() for value in field(body, "IDs").split(",") if value.strip()]

if not item_ids:
    raise ValueError("Solicitação incompleta")

radar = json.loads(NEWS_PATH.read_text(encoding="utf-8"))
items_by_id = {candidate.get("id"): candidate for candidate in radar.get("items", [])}
items = [items_by_id[item_id] for item_id in item_ids if item_id in items_by_id]
if not items:
    raise ValueError("Nenhuma notícia da solicitação foi encontrada no radar atual")
...
for position, item in enumerate(items, start=1):
    print(f"Analisando {position}/{len(items)}: {item.get('title', '')[:80]}")
    user = f"""ID interno: {item.get('id')}
Título localizado: {item.get('title')}
Fonte informada: {item.get('source')}
...
Abrangência: {item.get('scope')}"""
    ...
    try:
        ...
        analysis = {
            "status": "revisao_manual",
            "reason": f"A análise não foi concluída nesta execução ({type(error).__name__}).",
            "analyzedAt": datetime.now(timezone.utc).isoformat(),
        }
