...
STREAMS={"Empresa em Pauta":["CEO empresa decisão estratégia OR demissão OR expansão","Operação e estratégia"],"Poder e regras":["projeto de lei empresas trabalho regulamentação OR fiscalização","Risco jurídico/regulatório"],"Mercado e trabalho":["empresas mercado emprego investimento juros Brasil","Custo e caixa"],"Relações internacionais":["Itamaraty diplomacia embaixada acordo comercial empresas Brasil","Mercado e reputação"],"Trabalho e representação":["sindicato convenção coletiva greve negociação empresas","Pessoas e trabalho"],"Pessoas e liderança":["liderança gestão pessoas saúde mental trabalho empresas","Pessoas e trabalho"],"NR-1 / AEP":["NR-1 AEP riscos psicossociais empresas","Risco jurídico/regulatório"],"IA e gestão":["inteligência artificial gestão empresas trabalho","Operação e estratégia"]}
...
def feed(q,n=4):
 u="https://news.google.com/rss/search?q="+quote(q+" when:14d")+"&hl=pt-BR&gl=BR&ceid=BR:pt-419"
...
items=[];seen=set()
for agenda,(query,impact) in STREAMS.items():
 for x in feed(query):
  ...
  angle=f"O que este fato pode mudar para empresas em {impact.lower()}? Separar fato confirmado, declaração da fonte e consequência gerencial antes de gravar."
  items.append({"id":re.sub(r"[^a-z0-9]+","-",key)[:90],"agenda":agenda,"impact":impact,"evidence":evidence,"sourceType":"Fonte jornalística ou institucional - confirmar origem","score":score,**x,"summary":"Notícia coletada para triagem. Abra a fonte e valide o fato antes de transformá-lo em análise.","angle":angle,"origin":origin(x["title"]),"use":use})
